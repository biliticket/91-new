...
function resolveDataDir() {
  const candidates = [
    ...
    // Netlify function bundle layouts (nft included_files)
    path.join(__dirname, "media_gallery", "data"),
    path.join(__dirname, "..", "media_gallery", "data"),
    path.join(__dirname, "..", "..", "media_gallery", "data"),
    // sometimes files land next to the function file
    path.join(process.cwd(), "media_gallery", "data"),
  ];
  ...
  console.warn(
    "media data dir not found, tried:",
    candidates.join(" | "),
    "cwd=",
    process.cwd(),
    "__dirname=",
...
let DATA_DIR = null;
function dataDir() {
  if (!DATA_DIR) DATA_DIR = resolveDataDir();
  return DATA_DIR;
}
...
function getMeta() {
  if (!metaCache) {
    metaCache = readJson(path.join(dataDir(), "meta.json"));
    metaCache.img_cdns = IMG_CDNS;
    metaCache.img_cdn = IMG_CDNS[0];
    metaCache.vid_cdn = VID_CDN;
    metaCache.data_dir = dataDir();
  }
  return metaCache;
}

function getVideos() {
  if (!videosCache) videosCache = readJson(path.join(dataDir(), "videos.json"));
  return videosCache;
}

function getMediaVideos() {
  if (!mediaVideosCache) {
    mediaVideosCache = readJson(path.join(dataDir(), "media_videos.json"));
  }
...
function getImageChunk(idx) {
  ...
  const file = path.join(dataDir(), `images_${String(idx).padStart(3, "0")}.json`);
...
function enrichVideo(it) {
  ...
  // Always sign fresh — never reuse baked play_url from export JSON (auth_key expires).
